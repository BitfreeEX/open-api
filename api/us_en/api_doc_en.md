#  Bitfree-official-API-docs

#### Official Documentation for the Bitfree APIs and Streams([简体中文版文档](https://github.com/BitfreeEX/open-api/blob/master/api/zh_cn/api_doc_cn.md))
...
Welcome to [Bitfree](http://www.bitfree.biz) API document for developers.
...
Before signing any request, you must generate an API key via [Bitfree’s official website ](http://www.bitfree.biz)【User Center】-【API】. After generating the key, there are three things you must bear in mind:
...
The root URL for REST access：``` https://api.bitfree.biz ```
...
|fee_is_user_Bitfree_coin|	Selective filling|	（Redundant fields, ignored）0，When the Bitfree has the platform currency, this parameter indicates whether to use the platform currency to pay the handling fee, 0 no, 1 yes.|
|api_key|	Must fill|	api_key|
|time|	Must fill|	time stamp|
|sign|	Must fill|	autograph|
...
|mass_place|	Selective filling|	[{side:"BUY",type:"1",volume:"0.01",price:"6400",fee_is_user_Bitfree_coin:"0"}, {}, …]<br>Meaning：<br>symbol:currency，example btcusdt<br>mass_place:Order parameter。side：direction（Direction of businessBUY、SELL），<br>--------------------------------type：type（1:limit order、2:market order）<br>--------------------------------volume：Purchase quantity（Polysemy，Multiplex field） type=1:Represents the quantity of sales and purchasestype=2:Buy means the total price，Selling represents the total number<br>--------------------------------price：Authorized unit price：type=2：No need for this parameter<br>--------------------------------fee_is_user_Bitfree_coin：(Redundant fields) When the Bitfree has a platform currency，This parameter indicates whether to use platform currency to pay handling fee, 0 no, 1 yes.|
...
public class WsTest {
    ...
    public static void main(String[] args) {
        try {
//wsurl 
            String url = "wss://ws.Bitfree.com/kline-api/ws";
...
//Thread does not end, waiting for new messages，www.Bitfree.com Generally, a new deal will return in about a minute
            while (true) {
                Thread.sleep(1000);
            }
